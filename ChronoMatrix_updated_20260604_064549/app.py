...
try:
    conn = pymysql.connect(host='localhost', user='root', password='Akash@123')
    cursor = conn.cursor()
    cursor.execute('CREATE DATABASE IF NOT EXISTS chrono_matrix')
    conn.commit()
    conn.close()
    app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:Akash%40123@localhost/chrono_matrix'
...
def handle_faculty():
    if request.method == 'POST':
        ...
        if faculty_username or faculty_password:
            ...
            if User.query.filter_by(username=faculty_username).first():
                return jsonify({"error": "Username already exists."}), 400
        home_department_id = admin_department_id or data['home_department_id']
...
def handle_subjects():
    if request.method == 'POST':
        data = request.json
        admin_department_id = current_admin_department_id()
        department_id = admin_department_id or data['department_id']
...
def update_subject(id):
    ...
    obj.name = data.get('name', obj.name)
    obj.subject_type = data.get('subject_type', obj.subject_type)
    if data.get('department_id'):
        if not department_allowed(data['department_id']):
            return scoped_access_error()
...
def handle_rooms():
    if request.method == 'POST':
        ...
        if room_type == 'Theory':
            duplicate_error = "This theory room already exists. Theory rooms cannot be duplicated."
        elif department_id:
...
def handle_theory_requirements():
    if request.method == 'POST':
        ...
        weekly_hours = int(data['weekly_hours'])
        academic_year = str(data.get('academic_year') or '2025-26').strip()
        if not subject_allowed(subject_id) or not faculty_allowed(faculty_id) or not room_allowed(room_id):
            return scoped_access_error()
...
def update_schedule(id):
    ...
    if not subject_allowed(obj.subject_id) or not faculty_allowed(obj.faculty_id) or (obj.asst_faculty_id and not faculty_allowed(obj.asst_faculty_id)) or not room_allowed(obj.room_id):
        db.session.rollback()
        return scoped_access_error()
...
def add_schedule():
    ...
    if not subject_allowed(data['subject_id']) or not faculty_allowed(data['faculty_id']) or (asst_faculty_id and not faculty_allowed(asst_faculty_id)) or not room_allowed(data['room_id']):
        return scoped_access_error()
    conflict = schedule_conflict_reason(
        db.session,
        data['subject_id'],
