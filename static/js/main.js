...
function currentPageDepartmentId() {
    const candidates = ['lr-dept', 'tr-dept', 'alter-department'];
    for (const id of candidates) {
        const field = document.getElementById(id);
        if (field && field.value && field.value !== 'default') return field.value;
    }
    const saved = localStorage.getItem('savedDept');
    return saved && saved !== 'default' ? saved : (localStorage.getItem('department_id') || '');
}

function roomDisplayLabel(room) {
    const department = room.department || (room.department_id ? `Dept ${room.department_id}` : 'Shared');
    return `${room.name} (${department}${room.type ? `, ${room.type}` : ''})`;
}

function roomPageLabel() {
    const path = window.location.pathname;
    if (path.includes('theory-lobby')) return 'Classroom';
    if (path.includes('lab-lobby')) return 'Lab Room';
    return 'Room';
}

function roomsForCurrentPage(rooms) {
    const path = window.location.pathname;
    const deptId = currentPageDepartmentId();
    let filtered = rooms;

    if (path.includes('lab-lobby')) {
        filtered = rooms.filter(room =>
            room.type === 'Lab' &&
            (!deptId || !room.department_id || String(room.department_id) === String(deptId))
        );
    } else if (path.includes('theory-lobby')) {
        filtered = rooms.filter(room =>
            room.type === 'Theory' &&
            (!deptId || !room.department_id || String(room.department_id) === String(deptId))
        );
    ...
    if (!deptId) return filtered;

    if (path.includes('theory-lobby')) {
        return filtered.sort((a, b) => {
            const aPriority = String(a.department_id) === String(deptId) ? 0 : (!a.department_id ? 1 : 2);
            const bPriority = String(b.department_id) === String(deptId) ? 0 : (!b.department_id ? 1 : 2);
    ...
    return filtered
        ...
        .filter((room, index, list) =>
            list.findIndex(item => String(item.name).trim().toLowerCase() === String(room.name).trim().toLowerCase()) === index
        );
}
...
async function globalGenerateTimetable(mode = 'full') {
    ...
    try {
        ...
        if (res) {
            const details = typeof res.labs === 'number'
                ? ` Lab blocks: ${res.labs} (${res.lab_slots ?? res.labs} hrs), Theory: ${res.theory_slots ?? res.theory} hrs.`
                : '';
            const missing = Array.isArray(res.missing) && res.missing.length
                ? ` Missing: ${res.missing.slice(0, 3).map(item => `${item.subject_code} ${item.missing} hr`).join(', ')}${res.missing.length > 3 ? '...' : ''}.`
                : '';
            showToast(`${res.msg}${details}${missing} Redirecting...`, missing ? 'error' : 'success');
...
async function openDeleteModal(type) {
    ...
    if(type === 'room') { title.innerText = `Delete ${roomPageLabel()}`; endpoint = '/api/rooms'; }
    if(type === 'sub') { title.innerText = 'Delete Subject'; endpoint = '/api/subjects'; }
    
    try {
        let data = await apiCall(endpoint);
        if (type === 'room') data = roomsForCurrentPage(data);
        let options = '';
        if(type === 'dept') options = data.map(d => `<option value="${d.id}">${escapeHtml(d.name)}</option>`).join('');
        if(type === 'faculty') options = data.map(d => `<option value="${d.id}">${escapeHtml(d.name)}</option>`).join('');
        if(type === 'room') options = data.map(d => `<option value="${d.id}">${escapeHtml(roomDisplayLabel(d))}</option>`).join('');
        if(type === 'sub') options = data.map(d => `<option value="${d.id}">${escapeHtml(d.code)} - ${escapeHtml(d.name)}</option>`).join('');
...
function getItemLabel(type, item) {
    if (type === 'room') return roomDisplayLabel(item);
...
async function renderAlterFields() {
    ...
    if (type === 'subject-timing') {
        ...
        const validRooms = alterRooms.filter(room => {
            if (item.type === 'Theory') {
                return room.type === 'Theory' && String(room.department_id) === String(deptId);
            }
            return room.type !== 'Theory' && (!room.department_id || String(room.department_id) === String(deptId));
        });
        fields.innerHTML = `
