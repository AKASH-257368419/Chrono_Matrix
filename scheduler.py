...
MORNING_THEORY_SLOT_ORDER = [0, 1, 2, 3, 4]
AFTERNOON_THEORY_SLOT_ORDER = [5, 6, 7]
THEORY_SLOT_ORDER = MORNING_THEORY_SLOT_ORDER + AFTERNOON_THEORY_SLOT_ORDER
LAB_START_ORDER = {
    3: [5, 0],
    2: [5, 6, 3, 0, 1],
    1: [5, 6, 7, 3, 4, 0, 1, 2]
...
def _valid_theory_slots(phase='all'):
    if phase == 'morning':
        return MORNING_THEORY_SLOT_ORDER
    if phase == 'afternoon':
        return AFTERNOON_THEORY_SLOT_ORDER
    return THEORY_SLOT_ORDER

def _schedule_theory_pass(db, theories, academic_year, slot_order, generated, progress, daily_counts):
    for theory in theories:
        hours_needed = max(0, int(theory.weekly_hours))

        for day in DAYS:
            if progress[theory.id] >= hours_needed:
                break

            sessions_today = daily_counts.get((theory.id, day), 0)
            if sessions_today >= MAX_THEORY_SESSIONS_PER_SUBJECT_PER_DAY:
                continue

            for slot in slot_order:
                if progress[theory.id] >= hours_needed or sessions_today >= MAX_THEORY_SESSIONS_PER_SUBJECT_PER_DAY:
                    break
                if day == 'Saturday' and slot >= 5:
                    continue
                ...
                if not check_conflict(
                    ...
                    db.session.flush()
                    progress[theory.id] += 1
                    generated['theory'] += 1
                    generated['theory_slots'] += 1
                    sessions_today += 1
                    daily_counts[(theory.id, day)] = sessions_today
...
def generate_timetable(db, academic_year='2025-26', mode='full', department_id=None):
    ...
    # Step 2: Give theory first claim on morning slots so concentration-heavy classes land early.
    ...
    theory_progress = {theory.id: 0 for theory in theories}
    theory_daily_counts = {}

    _schedule_theory_pass(
        db,
        theories,
        academic_year,
        _valid_theory_slots('morning'),
        generated,
        theory_progress,
        theory_daily_counts
    )

    # Step 3: Generate Lab Timetable. Lab starts still prefer afternoon blocks.
    ...
    # Step 4: Fill only the remaining theory hours after labs are placed.
    _schedule_theory_pass(
        db,
        theories,
        academic_year,
        _valid_theory_slots('afternoon'),
        generated,
        theory_progress,
        theory_daily_counts
    )
    ...
    for theory in theories:
        ...
        hours_scheduled = theory_progress.get(theory.id, 0)
        if hours_scheduled < hours_needed:
            generated['missing'].append(_missing_item(
                'Theory',
                theory,
                hours_needed,
                hours_scheduled,
                'Morning slots were preferred first; remaining free slots were not enough.'
            ))
